...
Invoke the **`autofix`** skill to remediate the following findings / target: $ARGUMENTS

Follow the autofix skill's procedure exactly:

1. Select a platform adapter by running each adapter's `detect` against the repo. If none match, stop and say so — autofix cannot route without an adapter.
